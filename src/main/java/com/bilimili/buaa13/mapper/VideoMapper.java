...
public interface VideoMapper extends BaseMapper<Video> {
    ...
    @Select("select * from video where status = #{status} and delete_date is null;")
    List<Video> selectAllVideoByStatus(int status);

    //随机返回count个对应状态的视频，除去已删除的
    @Select("select * from video where status = #{status} and delete_date is null order by RAND() LIMIT #{count};")
