...
--
-- Host: 47.113.150.190    Database: bilimili
-- ------------------------------------------------------
...
CREATE TABLE `user` (
    ...
    #`author` tinyint(4) NOT NULL DEFAULT '0' COMMENT '官方认证 0普通用户 1个人认证 2机构认证',
...
/*!50503 SET character_set_client = utf8mb4 */;
CREATE TABLE `update` (
                           `id` int(11) NOT NULL AUTO_INCREMENT COMMENT '动态主id',
                           `vid` int(11) NOT NULL COMMENT '关联的视频id',
                           `uid` int(11) DEFAULT NULL COMMENT '发送者id',
                           `root_id` int(11) NOT NULL DEFAULT '0' COMMENT '根节点评论的id,如果为0表示为根节点',
                           `parent_id` int(11) NOT NULL COMMENT '被回复的评论id，只有root_id为0时才允许为0，表示根评论',
                           `to_user_id` int(11) NOT NULL COMMENT '回复目标用户id',
                           ...
                           PRIMARY KEY (`id`),
                           UNIQUE KEY `id` (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=0 DEFAULT CHARSET=utf8mb4 COMMENT='动如表';
/*!40101 SET character_set_client = @saved_cs_client */;
